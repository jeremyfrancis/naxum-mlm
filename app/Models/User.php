...
class User extends Authenticatable
{
    ...
    {
        return $this->categories()->where('id', 1)->exists();
    }
    ...
    {
        return $this->categories()->where('id', 2)->exists();
    }

    /**
     * Get the number of distributors referred by this user by a specific date.
     */
    public function getReferredDistributorsCount($date)
    {
        return $this->referrals()
            ->whereHas('categories', function($query) {
                $query->where('id', 1); // Distributor category
            })
